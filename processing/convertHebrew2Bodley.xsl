...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="cat" select="'Hebrew'"/>
  ...
  <xsl:variable name="path">
    <xsl:value-of
      select="concat('file:///home/jamesc/git/hebrew-mss/working/new/?select=', $files,';on-error=warning;recurse=',$recurse)"
    />
  </xsl:variable>
  ...
  <xsl:template name="main">
    ...
    <xsl:for-each select="$doc">
      ...
      <xsl:variable name="outputFilename"
        select="concat('file:///home/jamesc/git/hebrew-mss/working/draft-updated/', $msID, '.xml')"/>
      <!-- create output file -->
