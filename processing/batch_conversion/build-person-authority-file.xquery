...
declare function local:normalizename($name as xs:string)
{
    let $normname as xs:string:= replace(replace(replace(replace(lower-case(translate(translate(replace($name, '\(.*\)', ''), '.,"*', ''), "'", "")), '^various ', ''), '^fragments* of ', ''), '^parts* of ', ''), '^(the|a|an) ', '')
    return normalize-space($normname)
};
            ...
            <listPerson>
{
    ...
    let $hebrewpeople := (
        for $x in collection('../../collections/?select=*.xml;recurse=yes')//tei:persName[not(ancestor::tei:revisionDesc or ancestor::tei:respStmt)]
            let $name := normalize-space(string-join($x//text(), ' '))
            return
            if ($x eq $skipids) then
                ...
                <person>
                    <name>{ $name }</name>
                    <name>{ local:normalizename($name) }</name>
